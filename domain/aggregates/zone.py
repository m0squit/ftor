...
class Zone(object):
    ...
    def _predict_rate_oil(self):
        ...
        self.report.mae_train = self.flood_model.error
        self.report.mae_test = LossFunctions.mae(df.loc['test']['watercut'].to_list(), result['watercut'])
        ...
        self._calc_prediction_metric()

    def _calc_prediction_metric(self):
        df = self.report.df_flood.copy()
        df['diff_rel_prod_oil'] = None
        df['diff_prod_oil'] = None
        df['diff_cum_prod_oil'] = None
        df_test = df.loc['test']
        prod_oil = df_test['prod_oil'].to_list()
        prod_oil_model = df_test['prod_oil_model'].to_list()
